...
class CafeCoordinatePart extends ConsumerWidget {
  final double editButtonWidth;
  const CafeCoordinatePart({Key? key, required this.editButtonWidth}) : super(key: key);
    ...
    return Column(
      children: [
            ...
            SizedBox(
              ...
              child: NaverMap(
                ...
                options: NaverMapViewOptions(
                  initialCameraPosition: state.isCoordinateEdited ?
                    NCameraPosition(target: state.latLng, zoom: Zoom.large) :
                    NCameraPosition(target: mapState.selectedCafe.latLng, zoom: Zoom.large),
                  liteModeEnable: true,
                ...
                onCameraIdle: state.isCoordinateEdited ? () => viewModel.setLatLng() : () {},
              ),
            ),
            Column(
        ...
        EditApplyButton(
          ...
          onPress: () {
            if(!state.isCoordinateEdited) viewModel.setLatLng();
            viewModel.setIsCoordinateEdited(!state.isCoordinateEdited);
          }
        )
      ],
    );
